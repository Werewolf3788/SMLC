/* ==========================================================================
   Active Version: 2026-07-26_18:15
   File: script.js
   Description: Core operational pipeline & application engine for community portal pages.
   Features:
   - Dynamic Title/HTML Data-Driven Town & Keyword Extraction Matrix
   - Centralized Configuration Pipeline via config.json with Fallback Support
   - Dynamic ScoreStream Widget Extraction from Remote JSON Matrix
...
/* === SECTION 1: Dynamic Town Configuration & Matrix Mapping === */
...
function getActiveTownConfig() {
    /* === Logic: Inspects HTML Attributes & <title> to identify active town === */
    // 1. Primary check: HTML element attribute <html data-town="..."> or <body data-town="...">
    ...
    // 2. Secondary check: Page <title> match
...
window.globalAppConfig = null; // Holds fetched config.json data globally
...
function getSmartCacheBuster() {
    /* === Logic: Hourly auto-expiring cache buster token (?v=timestamp) === */
...
async function processDataPipelines() {
    ...
    try {
        ...
        console.log("Master config.json loaded successfully:", window.globalAppConfig);
    } catch(e) {
        console.error("Master config.json fetch fault, relying on secondary endpoints", e);
    }
    ...
    // 10. Dynamic ScoreStream Sports Integration with Primary config.json Pipeline
    try {
        let targetWidgetId = "68601"; // Fallback default
        const activeKey = ACTIVE_TOWN.jsonKey || "louisville";

        // Check master config.json first
        if (window.globalAppConfig && window.globalAppConfig.sports && window.globalAppConfig.sports.widgets) {
            const sportsWidgets = window.globalAppConfig.sports.widgets;
            const townWidgetGroup = sportsWidgets[activeKey] || sportsWidgets["clay_county_teams"];
            if (townWidgetGroup && townWidgetGroup.banner_id) {
                targetWidgetId = townWidgetGroup.banner_id;
            }
        } else {
            // Secondary fallback to sports.json if config.json sports payload is missing
            const sportsJsonUrl = 'https://raw.githubusercontent.com/Werewolf3788/Testpages/main/json/sports.json?' + cb;
            const sportsRes = await fetch(sportsJsonUrl);
            const sportsConfig = await sportsRes.json();
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    console.log(`Portal Engine initialized for ${ACTIVE_TOWN.primaryName}. Active Version: 2026-07-26_18:15`);
});
