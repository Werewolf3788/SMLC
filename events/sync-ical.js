...
// Active Version: v1.0.0 | Timestamp: 2026-07-29_15:37:00
// CSS / JS Imports: ?v=20260729_153700
...
// Initialize Firebase Admin with GitHub Secret Environment Variable
...
async function syncICalToFirestore() {
  ...
  try {
    ...
    console.log(`Parsed ${activeEvents.length} events strictly within the 720-hour window.`);

    // Wipe-and-replace Firestore collection: /smlc_events
    ...
    await batch.commit();
    console.log(`Successfully synced ${activeEvents.length} events to Firestore /smlc_events`);
    process.exit(0);

  } catch (err) {
    console.error("CRITICAL ERROR syncing iCal feed:", err);
    process.exit(1);
  }
}
